...
suite.addTest({name: 'plain object operations'}, (pass, fail) => {
	...
	if (events[2].type !== 'update' || events[2].path.join('.') !== 'address' || events[2].oldValue !== null || events[2].value !== tmpAddress || events[2].object !== po) fail('event 2 did not fire as expected');
...
suite.addTest({name: 'sub tree object operations'}, (pass, fail) => {
	...
	if (events[0].type !== 'update' || events[0].path.join('.') !== 'address' || events[0].oldValue !== null || events[0].value !== newAddress || events[0].object !== po) fail('event 0 did not fire as expected');
